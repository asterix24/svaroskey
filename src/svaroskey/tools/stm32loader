...
class CommandInterface:
    def open(self, aport='/dev/ttyUSB0', abaudrate=115200) :
        ...
        self.sp.setDTR(1) #disabilito reset
        self.sp.setRTS(1) #disabilito boot0
    ...
    def cmdExtendedErase(self, sectors = None):
        if self.cmdGeneric(0x44):
            mdebug(10, "*** Extented erase memory command")
            if sectors is None:
                # Global erase
                self.sp.write(chr(0xFF))
                self.sp.write(chr(0xFF))
                ...
                # Sectors erase
                crc = 0xFF
                crc = crc ^ (((len(sectors)-1) >> 8) & 0xFF)
                crc = crc ^ ((len(sectors)-1) & 0xFF)
                self.sp.write(chr(((len(sectors)-1) >> 8) & 0xFF))
                self.sp.write(chr(((len(sectors)-1) & 0xFF)))
                for c in sectors:
                    crc = crc ^ (c >> 8)
                    self.sp.write(chr((c >> 8) & 0xFF))
                    crc = crc ^ (c & 0xFF)
                    self.sp.write(chr(c & 0xFF))
                self.sp.write(chr(crc))
...
def usage():
    ...
    -V          Verbose
    -e          Erase
    -u          Readout unlock this command perform a masse erase on flash before to unlock
    -l          Readout lock
    -w          Write
    -v          Verify
    -r          Read
    -s length   Length of read
...
# http://www.python.org/doc/2.5.2/lib/module-getopt.html
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hqVewulvrp:b:a:s:")
...
#    cmd.cmdWriteProtect([0, 1])
        ...
        if conf['unlock']:
            mdebug(0, "Unlock the cpu memory READ. (NOTE the flash memory will erase all.)")
            cmd.cmdReadoutUnprotect()

        if conf['erase']:
            mdebug(0, "Erase all memory")
            if bootversion < 0x30:
                cmd.cmdEraseMemory()
            else:
                cmd.cmdExtendedErase()
        ...
        if conf['lock']:
            mdebug(0, "Enable READ Protection.")
